...
void IPObfuscationContext::SurveyFunction(Function &F) {
  ...
  for (const Use &U : F.uses()) {
    CallBase *CS = dyn_cast<CallBase>(U.getUser());
    if (!CS || !CS->isCallee(&U)) {
      return;
    }
  }
...
Function *IPObfuscationContext::InsertSecretArgument(Function *F) {
  ...
  while (!F->use_empty()) {
    ...
    CallBase::op_iterator I = CB->arg_begin();
    unsigned i = 0;
    ...
    for (CallBase::op_iterator E = CB->arg_end(); I != E; ++I, ++i) {
      Args.push_back(*I);
